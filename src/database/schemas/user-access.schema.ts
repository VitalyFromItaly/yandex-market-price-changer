...
export class UserAccess {
  ...
  /**
   * Пофичный доступ: какие возможности бота администратор открыл или закрыл
   * ЭТОМУ пользователю. Ключи — `TFeatureKey` из `features.domain.ts`.
   *
   * Хранятся ТОЛЬКО явные решения. Отсутствующий ключ — не «выключено», а «не
   * настраивали»: значение берётся из `FEATURE_META[key].defaultEnabled`.
   * Поэтому появление флагов не потребовало разовой миграции и существующие
   * продавцы ничего не потеряли.
   *
   * Поле здесь, а не отдельной коллекцией (как у ReportSchedule), потому что
   * читается оно на КАЖДОМ апдейте, дошедшем до отчёта, а запись доступа гейт
   * и так уже держит в руках. Вторая коллекция означала бы второй запрос в
   * Mongo там, где сейчас хватает одного.
   */
  @Prop({ type: Object })
