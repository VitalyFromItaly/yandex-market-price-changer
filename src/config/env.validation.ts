...
export const envValidationSchema = Joi.object({
  ...
  /*
   * Токен админского HTTP-API (журнал действий). Необязателен, но НЕзаданный
   * означает «API закрыто», а не «API без пароля»: AdminApiGuard в этом случае
   * отклоняет всех. Журнал содержит id и сообщения пользователей.
   *
   * Минимум 24 символа: короткий токен здесь — единственный барьер перед
   * чужой перепиской, и «admin» в качестве значения не должен проходить.
   */
  ADMIN_API_TOKEN: Joi.string().min(24).empty('').optional().messages({
    'string.min': 'ADMIN_API_TOKEN: минимум 24 символа — это единственная защита журнала',
  }),
