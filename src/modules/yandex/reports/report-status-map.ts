...
/**
 * Статусы заявок FBY (supply-requests), значимые для сортировки в сводке FBY.
 *
 * Живут ЗДЕСЬ, а не рядом с fby-сообщением, по той же причине, что SUBSIDY_TYPE:
 * литерал `CANCELLED` совпадает со статусом ЗАКАЗА, и тест «статусы не размазаны
 * по коду» справедливо ловит его в любом другом файле модуля yandex. Это
 * ДРУГОЙ набор статусов (у заявки на вывоз/утилизацию свой жизненный цикл, шире
 * enum openapi), поэтому — отдельная константа, а не переиспользование
 * ORDER_STATUS. Полный список подписей — в fby-message.ts; здесь только те коды,
 * по которым сводка расставляет приоритет.
 */
export const SUPPLY_STATUS = {
  /** Готово физически забрать со склада — самое важное для продавца. */
  READY_TO_WITHDRAW: 'READY_TO_WITHDRAW',
  READY_FOR_UTILIZATION: 'READY_FOR_UTILIZATION',
  /** Терминальные — уходят в конец списка. */
  CANCELLED: 'CANCELLED',
  FINISHED: 'FINISHED',
} as const;
