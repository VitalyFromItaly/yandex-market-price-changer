...
/**
 * Разбиение диапазона на окна по 30 дней — для истории по businesses, где
 * ограничение на ДЛИНУ диапазона есть, а на глубину нет.
 *
 * Отчёты её НЕ используют, хотя период тоже режут (periodWindows в
 * report-period.ts): здесь арифметика в миллисекундах над `Date`, а там —
 * календарная, по московским суткам. Смешивать нельзя: сутки не всегда ровно
 * 24 часа, и именно на этом рождаются ошибки «на один день» (см. moscow-day.ts).
 */
