 ...
 * короткие, а исходящее — отчёт на десяток строк. На 200 (и даже 500) символах
 * от отчёта остаётся заголовок, по которому нельзя понять, что именно не
 * понравилось продавцу, — а журнал заводили ровно за этим. Потолок — 4096:
 * ровно столько влезает в одно сообщение Telegram, то есть отчёт хранится
 * целиком, а не куском.
 */
export const MAX_OUTGOING_LENGTH = 4096;
