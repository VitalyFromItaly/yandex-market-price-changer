...
import { TelegramUserService } from './services/user.service';
import { TelegramScenesModel } from '../../../database/mongo';

@DecorateMethodsWith(TryCatch())
export default class BaseTelegramBot<T> implements ITelegramBot<T> {
  ...
  protected onStart(message = 'Нажми кнопку "Показать команды" (справа), чтобы увидеть список команд.') {
    ...
    this.bot.start(async (ctx) => {
      const user = await this.userService.handleUser(ctx.from, this.instanceName as EBotName);
      await TelegramScenesModel.deleteMany();
  ...
  public notifySubscribers(payload: T) {
    throw new Error('Method not implemented.');
  }
